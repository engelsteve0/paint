...
public class PaintApplication extends Application {
    ...
    public void start(Stage stage) throws IOException {
        ...
        this.zoom(canvas);  //sets up zoom properties
    ...
    private static void openFile(File file) {   //actually opens an image file
        try {
            ...
            lastSaved = file;                           //keeps track that this file should be saved to
            Image image = new Image(String.valueOf(file));
            canvas.setHeight(image.getHeight());        //adjust canvas according to image size
            canvas.setWidth(image.getWidth());
            canvas.getGraphicsContext2D().drawImage(image, 0, 0, image.getWidth(), image.getHeight()); //actually draws image
            stage.setTitle("Paint: " + file); //renames title to reflect name of file
            canvas.setScaleX(1);            //sets canvas scale to default to avoid losing image
            canvas.setScaleY(1);
            sp.setHmax(1);
            sp.setVmax(1);
            sp.setHmin(0);
        ...
        if(saveFile==null){      //if a file hasn't been saved before, prompt user to save as.
            ...
            if (saveFile != null) {
                try {
                    ...
                    canvas.setScaleX(ogx);
                    canvas.setScaleY(ogy);  //sets canvas scale to its original size
                } catch (Exception ex) {
    ...
    public static void newImage() { //sets up a blank canvas
        canvas.setWidth(128);       //default 128x128, like in regular MS Paint
        ...
        lastSaved = null;                                               //fixes a bug whereby user could work on a new image and then automatically save/overwrite an old image
        if(sp!=null){
            sp.setHmax(1);
            sp.setVmax(1);
            sp.setHmin(0);
    ...
    public void zoom( Canvas canvas) { //handles zooming/scaling with mouse
        canvas.setOnScroll(
                new EventHandler<ScrollEvent>() {
                    ...
                    public void handle(ScrollEvent event) {
                        if(event.isControlDown()) { //must have control down
                            ...
                            canvas.setScaleX(canvas.getScaleX() * zoomFactor);  //resizes canvas appropriately
